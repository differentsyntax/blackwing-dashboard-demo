...
const TradingChart: React.FC<TradingChartProps> = ({
    ...
    return (
      <>
        <line
          ...
          x2={width}
          y2={height}
          className="stroke-slate-500 w-1"
        />
        {xLabels}
      </>
    );
  ...
  return (
    <svg
      width={width + 120}
      height={height + 20}
      viewBox={`0 0 ${width + 30} ${height + 40}`}
      className={`bg-zinc-900 border border-gray-800 rounded-lg shadow-lg xs:col-span-1 sm:col-span-1 md:col-span-3 lg:col-span-3 p-6`}
    >
      {renderGridLines()}
      {renderXAxis()}
      {renderYAxis()}
